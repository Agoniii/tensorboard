...
module tf.graph.render {
  ...
  export class RenderGraphInfo {
    ...
    constructor(hierarchy: hierarchy.Hierarchy, displayingStats: boolean) {
      ...
      this.root = new RenderGroupNodeInfo(
        hierarchy.root,
    ...
    private colorHistogram(
      ...
      if (Object.keys(histogram).length > 0) {
        ...
        const numItems = _.sum(
          Object.keys(histogram).map((key) => histogram[key])
        );
    ...
    getOrCreateRenderNodeByName(nodeName: string): RenderNodeInfo {
      ...
      if (node.stats) {
        ...
        renderInfo.computeTimeColor = this.computeTimeScale(
          node.stats.getTotalMicros()
        );
      }
      ...
      if (deviceHistogram) {
        renderInfo.deviceColors = this.colorHistogram(
          deviceHistogram,
          this.deviceColorMap
        );
      }
    ...
    isNodeAuxiliary(renderNode: RenderNodeInfo): boolean {
      let parentNode = <RenderGroupNodeInfo>(
        this.getRenderNodeByName(renderNode.node.parentNode.name)
      );
    ...
    buildSubhierarchy(nodeName: string): void {
      ...
      _.each(bridgegraph.edges(), (bridgeEdgeObj) => {
        ...
        let isHighDegreeControlEdge =
          !bridgeMetaedge.numRegularEdges &&
        ...
        let otherDegreeCount = (inbound ? otherCounts.out : otherCounts.in)[
          otherName
        ];
        ...
        if (adjoiningMetaedge && !bridgeMetaedge.numRegularEdges) {
          ...
          let ordering = this.hierarchy.getTopologicalOrdering(
            topGroupNode.name
          );
      ...
      _.each([true, false], (inbound) => {
        ...
        _.each(coreGraph.nodes(), (childName) => {
          ...
          let structuralNodeName = getBridgeNodeName(
            inbound,
            nodeName,
            'STRUCTURAL_TARGET'
          );
  ...
  export class AnnotationList {
    ...
    push(annotation: Annotation): void {
      ...
      this.list.push(
        new Annotation(
          ellipsisNode,
          new RenderNodeInfo(ellipsisNode),
          null,
          AnnotationType.ELLIPSIS,
          annotation.isIn
        )
      );
    }
  ...
  function setGroupNodeDepth(
    renderInfo: RenderGroupNodeInfo,
    depth: number
  ): void {
  ...
  function createShortcut(
    ...
    if (
      (src.node.include === InclusionType.INCLUDE ||
  ...
  function extractSpecifiedNodes(renderNode: RenderGroupNodeInfo) {
    ...
    _.each(graph.nodes(), (n) => {
      ...
      if (
        ...
        if (
          renderNode.coreGraph.outEdges(n).length >
  ...
  function extractHighInOrOutDegree(renderNode: RenderGroupNodeInfo) {
    ...
    _.each(graph.nodes(), (currentNode) => {
      ...
      let inDegree = _.reduce(
        graph.predecessors(currentNode),
        (inDegree, pred) => {
      ...
      let outDegree = _.reduce(
        graph.successors(currentNode),
        (outDegree, succ) => {
    ...
    for (
      let i = validNodeCount - 1;
      nodeToInDegree[sortedByInDegree[i]] > inDegreeUpperBound;
      i--
    ) {
    ...
    let sortedByOutDegree = Object.keys(nodeToOutDegree).sort(
      (node0, node1) => {
    ...
    for (
      let i = validNodeCount - 1;
      nodeToOutDegree[sortedByOutDegree[i]] > outDegreeUpperBound;
      i--
    ) {
