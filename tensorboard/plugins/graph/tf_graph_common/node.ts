...
module tf.graph.scene.node {
    ...
    switch (colorBy) {
      case ColorBy.STRUCTURE:
        if (renderInfo.node.type === NodeType.META) {
          ...
          _.isNumber((renderInfo.node as OpNode).functionOutputIndex)
        ) {
  ...
  export function updateInputTrace(
    ...
    svg
      .selectAll('.input-edge-highlight')
      .classed('input-edge-highlight', false);
    svg
    ...
    let visibleNodes = _findVisibleParentsFromOpNodes(
      renderGraphInfo,
      highlightedNodes
    );
  ...
  function traceAllInputsOfOpNode(
    ...
    _.each(inputs, function(nodeInstance) {
      ...
      if (resolvedNode instanceof MetanodeImpl) {
        ...
        resolvedNode = renderGraphInfo.getNodeByName(
          resolvedNodeName
        ) as OpNode;
      }
  ...
  function _createVisibleTrace(
    ...
    _.each(destinationParentPairs, function(value) {
      ...
      let outer = value[1];
      let edgeSelector =
        `[data-edge="${inner.name}--${startNodeTopParentName}` +
        `~~${outer.name}~~OUT"]`;
    ...
    for (let index = 1; index < startNodeIndex; index++) {
      ...
      let edgeSelector =
        `[data-edge="${targetNodeTopParentName}~~${outer.name}` +
        `~~IN--${inner.name}"]`;
      svg.selectAll(edgeSelector).classed('input-edge-highlight', true);
    }
  }
   ...
   * @private
   */
  function _findVisibleParentsFromOpNodes(
    renderGraphInfo,
    nodeNames: string[]
  ) {
  ...
  function _markParentsOfNodes(
    svgRoot: SVGElement,
    visibleNodes: {[nodeName: string]: Node}
  ) {
  ...
  export function getVisibleParent(
    ...
    while (!found) {
      ...
      if (currentParent === undefined) {
        ...
        let renderNode = renderGraphInfo.getRenderNodeByName(
          currentParent.name
        );
